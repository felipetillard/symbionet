...
export default function StorefrontClient({ products, whatsappNumber, storeName }: StorefrontClientProps) {
  ...
  const generateWhatsAppMessage = () => {
    ...
    let message = `¡Hola! Me gustaría hacer un pedido de ${storeName}:\n\n`;
  ...
  if (!products || products.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-white mb-2">No Hay Productos Disponibles</h3>
        <p className="text-white/60">¡Vuelve más tarde para ver nuevos productos!</p>
      </div>
    );
  }
              ...
              <div className="px-4 pb-4">
                  ...
                  {!isOutOfStock && whatsappNumber ? (
                    ...
                    <div className="text-center p-3 bg-orange-500/10 border border-orange-500/30 rounded-lg">
                      <p className="text-orange-300 text-sm">Pago no disponible</p>
                    </div>
                  ) : (
                    <div className="text-center p-3 bg-white/5 border border-white/10 rounded-lg">
                      <p className="text-white/40 text-sm">Sin Stock</p>
                    </div>
                  )}
                </div>
              </div>
